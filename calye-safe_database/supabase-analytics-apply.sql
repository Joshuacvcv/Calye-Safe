-- ============================================================================
-- Calye-Safe — APPLY DATA-DRIVEN ANALYTICS + pg_cron JOB
--
--   PASTE THIS WHOLE FILE INTO:  Supabase Dashboard > SQL Editor > New query > Run
--
--   What it does:
--     1. Enables the pg_cron extension (safe to re-run).
--     2. Creates v_incident_density, fn_incident_summary, fn_incident_density,
--        monthly_reports, and fn_monthly_summary (all idempotent).
--     3. Schedules the automated monthly report on the 1st of every month at 08:00
--        (re-runnable — any previous schedule with this name is replaced).
--     4. Runs verification queries so you can confirm the result.
--
--   To STOP the auto-job later, run:   select cron.unschedule('calye-monthly-report');
--   To KEEP the job but disable auto-generation, delete the DO  ...  block below
--   before running (the rest of the file is unaffected).
-- ============================================================================
-- PART 1 — ANALYTICS LAYER (idempotent; safe to re-run)
--   Creates v_incident_density, fn_incident_summary, fn_incident_density,
--   monthly_reports, and fn_monthly_summary. All functions are SECURITY
--   DEFINER so they keep working after RLS is enabled; EXECUTE is granted to
...
--   Job name : calye-monthly-report
--   Schedule : 1st day of every month at 08:00 (generates the PREVIOUS month)
-- ============================================================================
...
begin
  begin
    perform cron.unschedule('calye-monthly-report');
  exception when others then null;
  end;

  perform cron.schedule('calye-monthly-report', '0 8 1 * *', $cron$
...
-- Sample summary for the current month (should return a JSON object):
select public.fn_incident_summary(date_trunc('month', now()), now()) as current_month_summary;

-- If the reports table is empty, the result above shows totals = 0 / [] arrays,
-- which is expected until the seed data is loaded (supabase-seed.sql).
